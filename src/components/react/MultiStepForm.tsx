...
export default function MultiStepForm() {
  ...
  const buildCalendarUrl = (priorityLevel: 'high' | 'medium' | 'low', data: typeof formData) => {
    const baseUrl = CALENDAR_URLS[priorityLevel];
    ...
    const params = new URLSearchParams({
      ...
      lead_score: data.lead_score.toString(),
    ...
    if (data.gclid) params.append('gclid', data.gclid);
    if (data.fbclid) params.append('fbclid', data.fbclid);
    if (data.msclkid) params.append('msclkid', data.msclkid);
  ...
  const nextStep = async () => {
    if (currentStep === 4) {
      ...
      try {
        ...
        // Update browser URL with contact info for iframe/tracking persistence
